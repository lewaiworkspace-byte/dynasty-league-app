 ...
 * renders with cents ("$1,500.33") so it is visible as the bug it is. Nothing
 * is rounded in either direction.
 *
 * Used by the restructure surfaces, where restructure_contract() and its
 * preview functions enforce whole dollars in the data. Do not adopt it
 * elsewhere to "fix" a fractional figure -- rule 1.9 whole-dollar rounding is
 * still an open league-wide decision, 161 contract-year rows carry fractional
 * signing-bonus proration, and eight of ten teams have cents in their 2026 cap
 * today. Those are real values, not defects, and formatMoney is correct for
 * them.
 *
 * @param {number|string|null|undefined} n
