...
// numeric     -- PRESENTATION: right-aligned, and width-hinted by .col-num
// numericSort -- COMPARISON: compare as a number rather than a string
// sortKey     -- when sorting needs a different field than the one drawn
//
// numeric and numericSort are separate because Status needs one and not the
// other. No column currently sets `numeric`; it is kept because the next
// column that is genuinely a figure will want it, and because the table reads
// it for the .col-num class.
export const REPORT_COLUMNS = [
  { key: 'full_name', label: 'Player' },
  { key: 'position', label: 'Pos' },
  { key: 'nfl_team', label: 'NFL' },
  // numericSort, NOT numeric. The two were one flag until the first real pull
  // showed what the column actually holds: 159 of 248 designations are the word
  // "Questionable", which is wider than the 88px .col-num hint and reads wrong
  // right-aligned beside nine columns of left-aligned text. `numeric` means
  // "draw it as a figure"; `numericSort` means "compare it as a number". Status
  // is the one column that wants the second without the first.
  { key: 'designation', label: 'Status', sortKey: 'severity', numericSort: true },
...
export function compareRows(a, b, col, dir) {
  ...
  let c = col.numeric || col.numericSort
    ? Number(av) - Number(bv)
