...
// 5. Every real year must also clear that season's league minimum salary
//    (on cash or cap, whichever is higher -- same rule as the Deion
//    check). A low target PPV combined with a shape that's thin in later
//    years (Max Control's roster-bonus-heavy tail is the usual case) can
//    fall short there even though PPV math alone is satisfied, because
//    league-minimum compliance is a real-dollar test, not a PPV one. Any
//    shortfall gets raised via non-guaranteed salary specifically -- the
//    same dollars satisfy the floor no matter which bucket they're in
//    (cash and cap count guaranteed and non-guaranteed identically), but
//    non-guaranteed carries a far lower PPV weight than guaranteed, so
//    this is the smallest possible push above the target. There's no
//    matching "infeasible below this PPV" refusal gate -- a naive sum of
//    each season's minimum salary looks like a natural floor but isn't
//    one: those dollars can be paid through non-guaranteed salary, which
...
export function generateContract(targetPPV, totalYears, philosophy, maxVoidYears, startYear) {
  ...
  // League minimum salary floor (rule 5 above). Uses the same unrounded
  // proration bidMath.js's validators use, so a top-up applied here is
  // guaranteed to also satisfy the validator's own check downstream.
  const span = T + chosenVoid;
  const proratedShare = signingBonusTotal / span;
  const floorTopUps = [];
  yearsRounded.forEach((y, idx) => {
    const seasonYear = Number(startYear) + idx;
    const floor = leagueMinimumSalary(seasonYear);
    const cash = (idx === 0 ? signingBonusTotal : 0) + y.guaranteedSalary + y.nonGuaranteedSalary + y.rosterBonus;
    const cap = proratedShare + y.guaranteedSalary + y.nonGuaranteedSalary + y.rosterBonus;
    const shortfall = floor - Math.max(cash, cap);
    if (shortfall > 1e-9) {
  ...
  const floorTopUpNote =
    floorTopUps.length > 0
      ? `Raised non-guaranteed salary to clear that season's league minimum salary: ${floorTopUps
          .map((t) => `${t.seasonYear} +$${t.amount}`)
          .join(', ')}.`
      : null;
