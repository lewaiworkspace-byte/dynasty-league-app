...
// Pages past PostgREST's 1,000-row default ceiling, which otherwise
// truncates silently with no error.
//
// The .order() is not cosmetic and must not be dropped: without an ORDER BY,
// Postgres makes no guarantee that row order is stable between queries, so
// consecutive .range() pages can overlap (double-counting rows) or skip rows
// entirely -- and the result would look completely normal while being wrong.
// That risk is highest for an aggregating view like this one, where the
// planner has the most freedom to vary output order.
//
// (player_id, season_year) is unique per row: this view's grain is one row
// per player per season, which is also why aggregateByPlayer() has several
// season rows to collapse per player. Both callers below query that same
// view; a future caller pointed at a different table would need its own
// stable, unique ordering here.
